                             ...
                             gleDouble xform_array[][2][3])   /* 2D contour xforms */
{
    ...
    mem_anchor = reinterpret_cast<char *>(malloc (4 * 3 * (size_t)ncp * sizeof(gleDouble) + 2 * 3 * (size_t)ncp * sizeof(double) + 2 * 1 * (size_t)ncp * sizeof(int)));
    ...
    while (inext < npoints - 1)
    {
        ...
        if (xform_array == NULL)
        {
            if (color_array == NULL)
            {
                ...
                else if (__TUBE_DRAW_FACET_NORMALS)
                {
