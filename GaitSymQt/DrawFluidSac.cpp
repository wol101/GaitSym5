...
//    qDebug() << "DrawFluidSac " << facetedObject->GetNumTriangles() << " triangles created\n";
    ...
    if (m_displayFluidSacForces)
    {
        ...
        for (size_t i = 0; i < pointForceList.size(); i++)
        {
            ...
            std::unique_ptr<FacetedObject> facetedPolyline = std::make_unique<FacetedPolyline>(&polyline, m_fluidSacForceRadius, m_fluidSacForceSegments, m_fluidSacForceColour, 1, true);
